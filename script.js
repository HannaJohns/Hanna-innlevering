...
function handleBubbles() {
    if (gameFrame % 100 == 0) {
        // Every 100th frame, push a new bubble
...
// Create an array to hold multiple enemies
const enemiesArray = [];
...
// Function to handle multiple enemies
const maxEnemies = 5; // Set the maximum number of enemies

function handleEnemies() {
    // Increase the interval from 100 to 200 frames and limit the number of enemies
    if (gameFrame % 200 == 0 && enemiesArray.length < maxEnemies) {
        enemiesArray.push(new Enemy());
    }
    for (let i = 0; i < enemiesArray.length; i++) {
        enemiesArray[i].draw();
        enemiesArray[i].update();
        if (enemiesArray[i].x < 0 - enemiesArray[i].radius * 2) {
...
function handleGameOver() {
    ctx.fillStyle = 'white';
    ctx.fillText('GAME OVER, you reached score ' + score, 80, 250);
    gameOver = true;
}
